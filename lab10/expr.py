...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        raise NameError('Name {self.string} not found in the current environment {env}!'.format(self=self, env=env))
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        eval_operands = [x.eval(env) for x in self.operands]
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        copy_parent_env = self.parent.copy()
        copy_parent_env.update(zip(self.parameters, arguments))
        return self.body.eval(copy_parent_env)
