...
def totals_tree(m):
    ...
    b = []
    if is_weight(end(left(m))) and is_weight(end(right(m))):
        b = [tree(total_weight(end(left(m)))),tree(total_weight(end(right(m))))]
    elif is_weight(end(left(m))):
        b = [tree(total_weight(end(left(m)))), totals_tree(end(right(m)))]
    elif is_weight(end(right(m))):
        b = [totals_tree(end(left(m))), tree(total_weight(end(right(m))))]
    else:
        b = [totals_tree(end(left(m))), totals_tree(end(right(m)))]
...
def make_fib():
    ...
    True
    """
    def fib_helper(n=0, n_1=0, n_2=0):
        def nextFib():
            nonlocal n, n_1, n_2
            if n <= 1:
                val = n
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    def withdraw_helper(amount, secret):
        return withdraw(amount, old_password) if secret == new_password else withdraw(amount, secret)

    validation = withdraw(0, old_password)
    # print("DEBUG: ", type(validation))
...
def div_interval(x, y):
    ...
    assert lower_bound(reciprocal_y) <= upper_bound(reciprocal_y), "Wrong"
